...
const ProductCard = ({ Id, Title, Desc, Url, Label, Price }) => {
  ...
  return (
    <>
      <div
        key={Id}
        className={`product animate-fade flex flex-col lg:flex-row items-center w-[100%] lg:w-[45%] h-auto lg:h-[200px] bg-green-50 rounded-xl my-2`}
      >
        {/* Product Image  */}
        ...
        <div className="product-content w-full lg:w-[50%] lg:h-full h-[50%]  gap-2 lg:gap-2 p-4 flex flex-col justify-center items-start ">
          ...
          <div className="btn-price py-1 flex flex-row justify-between lg:flex-col gap-2 items-center lg:items-start w-full">
            ...
            <button
              onClick={ClickFunc}
              className="py-[4px] px-3 text-[12px] lg:font-bold text-white rounded-full animate-pop bg-primary"
            >
              Add to cart
            </button>
          </div>
